...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-3.30.5-collection-auth.patch # Prerequisite for evolution-ews cert checking fix
...
src_install() {
	if [[ -f /usr/etc/xdg/autostart/org.gnome.Evolution-alarm-notify.desktop ]]; then
		addwrite /usr/etc/xdg/autostart/org.gnome.Evolution-alarm-notify.desktop
		rm /usr/etc/xdg/autostart/org.gnome.Evolution-alarm-notify.desktop || die
	fi
